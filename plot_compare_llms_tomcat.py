...
OUTPUT_PNG = 'comparativo_llms_tomcat.png'
...
# Construir máscara para tomcat: procurar 'tomcat' em CVE ou Arquivos Analisados quando existirem
if cve_col is not None:
    mask_cve = all_df[cve_col].astype(str).str.contains('tomcat', case=False, na=False)
else:
    mask_cve = pd.Series([False]*len(all_df))

if files_col is not None:
    mask_files = all_df[files_col].astype(str).str.contains('tomcat', case=False, na=False)
else:
    mask_files = pd.Series([False]*len(all_df))

mask = mask_cve | mask_files

tomcat_df = all_df[mask].copy()

if tomcat_df.empty:
    print('Nenhum CVE relacionado a "tomcat" encontrado nas abas. Vou usar todos os dados para gerar um gráfico geral.')
    tomcat_df = all_df.copy()
...
for candidate in ['Vulnerabilidade Detectada', 'Vulnerabilidade?', 'Vulnerabilidade']:
    if candidate in tomcat_df.columns:
        vuln_col = candidate
        break
...
tomcat_df[vuln_col] = tomcat_df[vuln_col].astype(str).str.upper().str.strip()
...
tomcat_df['Vuln_Normalizada'] = tomcat_df[vuln_col].apply(normalize_vuln)

# Agrupar por modelo e status
group = tomcat_df.groupby(['Modelo_Sheet', 'Vuln_Normalizada']).size().reset_index(name='count')
...
plt.title('Comparativo de Detecções por LLM (tomcat)')
